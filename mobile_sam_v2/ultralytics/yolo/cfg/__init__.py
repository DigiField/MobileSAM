...
from mobile_sam_v2.ultralytics.yolo.utils import (DEFAULT_CFG, DEFAULT_CFG_DICT, DEFAULT_CFG_PATH, LOGGER, ROOT, USER_CONFIG_DIR,
...
def handle_yolo_hub(args: List[str]) -> None:
    ...
    if args[0] == 'login':
        ...
        # Log out from mobile_sam_v2.ultralytics HUB
        hub.logout()
...
def entrypoint(debug=''):
    ...
    if 'rtdetr' in model.lower():  # guess architecture
        from mobile_sam_v2.ultralytics import RTDETR
        model = RTDETR(model)  # no task argument
    elif 'sam' in model.lower():
        from mobile_sam_v2.ultralytics import SAM
        model = SAM(model)
    else:
